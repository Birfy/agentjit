...
2. **只承诺真的会检查的东西。** 早先这里写着"分支覆盖率必须 100%""会拿几百个
   输入喂进来"—— 那些关卡后来删掉了，话却留着，等于拿不存在的规则吓唬模型。
   现在分成两段：会判不通过的（静态检查、你的用例），和只是经验之谈的。
"""
...
1. **静态检查没过**（上面那些禁用项）。
2. **没通过给你的测试用例**：输入喂进去，返回值要和期望的完全一致。

# 另外几条经验（不会直接判你不通过，但十有八九会让你挂在用例上）

- **别崩。** 空数组、空字符串、缺失的可选字段、异常格式的数值 —— 用例里大概率有。
- **要确定。** 同样的输入跑两遍得到一样的结果。不要遍历 set 后直接返回（顺序会变），
  不要用当前时间和随机数。要顺序稳定就显式 sorted()。
- **别写防御性死分支。** `if not rows: return {{}}` 这种，后面的循环本来就能处理
  空输入。代码越小越好。
- **只实现需求要求的东西。** 不要自作主张加校验、加日志、加兜底。
