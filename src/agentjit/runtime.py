...
from .registry import Function, Probe, Registry, Version
...
class Runtime:
    ...
    def call_many(self, handle: str, args_list: list[dict[str, Any]]) -> list[CallOutcome]:
        ...
        v = fn.best()
        if v is None:
            why = next((x.quarantine_reason for x in fn.versions if x.quarantine_reason), "")
            return [CallOutcome(False, kind="quarantined",
                                message=f"{fn.handle} 的所有版本都被隔离了。{why}")
                    for _ in args_list]
