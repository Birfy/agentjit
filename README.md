...
`tools/audit_tests.py` 是这个实验的脚本，可以重跑。协议：需求、种子、**参考实现**
三样一起写死，写的时候还没见过任何生成用例；参考实现按需求原文直译，是这次审计的
oracle。

| 需求形态 | 一致 | 我埋的坑碰到了几个 |
| --- | --- | --- |
| 日志行解析 | 8/8 | 3/3（消息里含 `]`、空行、首尾空白） |
| 工作日计数 | 8/8 | 3/3（start 晚于 end、start==end、端点在周末） |
| 嵌套压平 | 8/8 | 3/3（空 dict 丢弃、数组不展开、三层嵌套） |
| 分组取前 N | 8/8 | 3/3（组内同薪、不足 2 人、多个组） |
| 带状态聚合 | 8/8 | 3/3（重复 start、孤立 stop、末尾未闭合） |
| 金额分摊 | 8/8 | 3/3（余数非 0、权重相同看下标、单权重） |
| **合计** | **48/48** | **18/18** |

48/48 一致本身说明不了什么 —— 如果模型只挑简单的写，一致率是白送的。**18/18
碰到了坑**才让这个数有意义。

#### 但这个实验有个盲点，是我自己造成的

上面 6 个需求是我写的，而且**刻意写精确了**。真实需求是含糊的，而含糊才是风险所在。
补了一轮专测这个的：给三句故意含糊的需求（"四舍五入"没说 0.5 往哪边、"去重"没说
按什么去重、"去掉空值"没说空字符串算不算空），看模型怎么办。结果分两半：

- **绕开**：只写 1.5 不写 2.5（1.5 在两种读法下答案相同，2.5 才分叉）；
  只测 `null` 不测空字符串/0/false。安全，但也没解决歧义。
- **默默断言**：去重那三个需求根本没提的决定，全被当成确定事实写进用例，
  note 里读起来像是需求本来就这么规定的。**这是真正危险的那种** ——
  调用方要是另一种读法，正确的实现会被这条用例判死。

所以加了 `assumes` 字段：需求没说清而模型自己定了的决定，**必须写出来**。
改完再跑同样三个需求，行为反过来了：2.5 写了（声明"向远离零方向舍"）、
`{a:0, b:false, c:"", d:null}` 写了（声明"空值仅指 null"）、去重那三个决定
逐条声明。挂在这种用例上时，报告直说**"这不是谁错了，是需求没说清"**。
...
- **修复循环没有被真实模型压力测试过。** 5 次合成 5 次一遍过，三次重试那条路
  一次也没走到。"结构化反馈到底有没有用"仍然是未知的。
- **补出来的用例只人工复核过一轮**（8/8 对）。样本是一个需求、一个模型。
  模型算错一个边界是常事，而一条错用例会把正确实现判死 —— 这是这套设计最大的风险。
