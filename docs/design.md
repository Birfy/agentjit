...
v0.1 里正确性是免费的：轨迹本身就是测试用例。现在没有轨迹了，这个缺口必须补。

三种补法：

| 方案 | 问题 |
| --- | --- |
| 让 LLM 自己造测试用例 | 造用例和写代码是同一个模型，错误相关。它误解了需求，就会造出"证明自己误解正确"的用例 |
| 生成完直接跑，agent 看结果 | 把验证责任推给 agent，而 agent 恰恰是在批量处理场景下最不会逐条看结果的 |
| **要求调用方给例子** | 需要调用方多花点力气 |

选第三个，并且把它变成硬规则：

> **无例子的合成产物不进持久缓存。**
> 给了例子 → `VERIFIED`，可缓存、可复用。
> 没给例子 → `EPHEMERAL`，只执行这一次，结果标记为未验证，用完即弃。

这是一条很干净的交换：**拿验收标准换复用权。** 它同时解决四个问题：

1. **消歧** — 两个例子比两段补充说明更能讲清需求。"amount 可能是 `"$1,200.50"`" 这种事，写在例子里一目了然，写在散文里模型会漏。
2. **测试集** — 合成循环的判定依据（§6.3）。
3. **schema 推断** — 参数和返回结构直接从例子结构读出来，不用猜。
4. **缓存命中的验收** — 这条最妙：**L2 语义命中的候选，用本次请求的例子跑一遍就知道能不能用。** 跑不过 = 不是同一个函数，视为 miss 去合成新版本。等于在每个复用点上都免费架了一道 guard，而且这道 guard 用的是本次调用方自己的标准。

调用方给不出例子怎么办？提供一个辅助动作：由模型从需求生成候选例子，**返回给调用方确认**。把"验证问题"降级成"确认问题"——让 agent 判断三对输入输出对不对，比让它审代码便宜得多，也可靠得多。

**例子的数量和质量要求**：至少 2 个，且必须包含一个边界情况（空输入、异常格式、零值之一）。只给 happy path 的例子，合成出来的代码在边界上必挂。这条应该在 `compile_function` 里做静态提示，而不是等运行时才发现。
...
| 等级 | 条件 | 可缓存 | 可复用 |
| --- | --- | --- | --- |
| `VERIFIED` | 过静态检查 + 全部例子通过（≥2 个，含边界） | ✅ | ✅ |
| `CONFIRMED` | 同上，且例子由调用方显式确认过 | ✅ | ✅ 优先 |
| `EPHEMERAL` | 无例子或例子不足 | ❌ | ❌ |
| `QUARANTINED` | 曾 `VERIFIED`，但运行时连续失败 | 保留 | ❌ |
...
**健康**
- `synth_success_rate` — 3 次尝试内通过例子的比例。目标 > 60%
...
| 阶段 | 内容 | 出口判据 |
| --- | --- | --- |
| **M0 — 能跑通** | `compile` + `call`；纯函数；子进程沙箱 + 静态检查；例子驱动的合成循环；内存缓存 | 手写一个真实需求，合成出正确代码并成功调用 |
| **M1 — 能复用** | 规格归一化；三级查找 + L2 复验；持久化 Registry；验证等级；`search`/`inspect` | 换一种说法描述同一需求能命中缓存；`net_savings` 转正 |
| **M2 — 能信任** | 完整 Guard；资源预算；QUARANTINE 与失效；MCP server 形态；审计日志 | 接进真实 agent 跑一个批处理任务，`silent_divergence` = 0 |
...
| **例子过拟合**——代码只在给的例子上对 | 强制边界例子；静态检查禁止硬编码例子输出；考虑留一个例子不参与合成、只做验收（**但例子本来就少，留出会削弱合成质量，这个权衡待实测**） |
...
1. **无例子场景到底有多大？** 整个设计押在"调用方愿意给例子"上。如果实际用起来 agent 普遍给不出例子，`EPHEMERAL` 会变成主路径，复用就没了。**这是 M0 最该先测的一件事**，而不是等 M2 才发现。
